...
Intercept = Standard Score

kindregular with aid, kindsmall, racehispanic, etc: Standard deviation between 95% confidence interval from the intercept for that variable

```{r}

# Fit model with both kind and race (no interaction)
model_kind_race <- linear_reg() |>
  fit(g4math ~ kind + race, data = x)
# Show tidy output with confidence intervals
model_kind_race |> tidy(conf.int = TRUE)
```

### Show the output of the code below to AI. Ask for help in interpreting it. What do the coefficients for kind and race suggest? 
...
### Are any effects statistically significant? How certain or uncertain should I be about these estimates?  

1. The intercept includes every race so it will be used as a baseline

2. It is not statistically significant because the common interval is zero

3. When is is small size then it isn't significant because it includes zero

4. Not significant, includes zero

5. Not significant, includes zero

6. Significant because it doesn't include zero

7. Significant because more years spent ties in with the math score

8. Not significant, includes zero

9. Not significant, includes zero

10. Not significant, includes zero

11. NO DATA

12. Not significant, includes zero

13. Not significant, includes zero
